...
export class KernelsProvider implements vscode.TreeDataProvider<KaggleKernelTreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<
    KaggleKernelTreeItem | undefined | null | void
  >();
  ...
  private hasFetchedRemote = false;

  constructor(private monitor: KernelStatusMonitor) {
    // Re-render tree without wiping pagination or remote lists
    this.monitor.onStatusChange(() => {
  ...
  async getChildren(
    element?: KaggleKernelTreeItem,
  ): Promise<KaggleKernelTreeItem[]> {
    if (!element) {
    ...
    // 1. Running / Tracked Kernels
    if (element.contextValue === "group_running") {
      const tracked = this.monitor.getTrackedKernels();
      if (tracked.length === 0) {
        const item = new KaggleKernelTreeItem(
          "No active kernels running",
    ...
    if (element.contextValue === "localKernel") {
      ...
      const codeItem = new KaggleKernelTreeItem(
        k.code_file,
        vscode.TreeItemCollapsibleState.None,
        "codeFile",
        k,
      ...
      if (k.codeFileExists) {
        codeItem.command = {
          command: "vscode.open",
          title: "Open Code",
          arguments: [k.codeFilePath],
        };
      }

      const metaItem = new KaggleKernelTreeItem(
      ...
      metaItem.command = {
        ...
        arguments: [k.metadataPath],
      };

      return [codeItem, metaItem];
    }

    // 3. Remote Kernels on Kaggle
    if (element.contextValue === "group_remote") {
      if (!this.hasFetchedRemote && !this.isLoadingRemote) {
        await this.fetchRemotePage();
      }

      if (this.remoteKernels.length === 0) {
        const emptyItem = new KaggleKernelTreeItem(
          "No remote kernels found",
          vscode.TreeItemCollapsibleState.None,
          "empty_remote",
        );
      ...
      const items: KaggleKernelTreeItem[] = this.remoteKernels.map((r) => {
        const ref = r.ref || r.title || "Untitled";
        const version = r.currentversionnumber || r.currentVersionNumber || "1";
        const runTime = r.lastruntime || r.lastRunTime || "recently";
        const votes = r.totalvotes || r.totalVotes || "0";
        const author = r.author || "Me";
        ...
        item.description = `v${version} (${runTime})`;
        item.tooltip = `Author: ${author}\nTotal Votes: ${votes}\nSlug: ${ref}`;
      ...
      if (this.hasMoreRemote) {
        ...
        moreItem.command = {
          command: "yaKaggle.loadMoreKernels",
          title: "Load More Kernels",
        };
        items.push(moreItem);
      }
  ...
  private async fetchRemotePage(): Promise<void> {
    ...
    try {
      const newItems = await KaggleCliService.listRemoteKernelsPage(
        this.currentPage,
        this.pageSize,
      );
      this.hasFetchedRemote = true;
      ...
      vscode.window.showErrorMessage(
        `Failed to fetch remote kernels: ${err.message}`,
      );
      this.hasMoreRemote = false;
    } finally {
