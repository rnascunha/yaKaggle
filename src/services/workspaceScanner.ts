...
export class WorkspaceScanner {
  public static async findLocalKernels(): Promise<LocalKernelMetadata[]> {
    ...
    // Sort alphabetically by ID, then by file path for stability
    return kernels.sort((a, b) => {
      const cmp = a.id.localeCompare(b.id, undefined, {
        sensitivity: "base",
        numeric: true,
      });
      return cmp !== 0
        ? cmp
        : a.metadataPath.fsPath.localeCompare(b.metadataPath.fsPath);
    });
  }
